CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS someaction FOR MODIFY
      IMPORTING keys FOR ACTION travel~someaction RESULT result.
ENDCLASS.
...
*************************************************************************************
  METHOD acceptTravel.
    ...
    IF keys[ 1 ]-%param-note = '123'.
      DO 8 TIMES.
        APPEND VALUE #(  %tky               = keys[ 1 ]-%tky
                         %state_area        = 'VALIDATE_NOTE' ) TO reported-travel.

        APPEND VALUE #( %tky = keys[ 1 ]-%tky ) TO failed-travel.

        APPEND VALUE #( %tky               = keys[ 1 ]-%tky
                        %state_area        = 'VALIDATE_NOTE'
                         %msg              = NEW /dmo/cm_flight_messages(
                                                                textid   = /dmo/cm_flight_messages=>enter_begin_date
                                                                severity = if_abap_behv_message=>severity-error )
                      %element-note = if_abap_behv=>mk-on ) TO reported-travel.
      ENDDO.
    ELSE.
      " modify travel instance
      MODIFY ENTITIES OF zrap100_r_traveltp_sol IN LOCAL MODE
         ENTITY Travel
         UPDATE FIELDS ( OverallStatus Note )
         WITH VALUE #( FOR key IN keys ( %tky           = key-%tky
                                          OverallStatus = travel_status-accepted
                                          Note = key-%param-note ) )  " 'A'
      FAILED failed
      REPORTED reported.
    ENDIF.
